function TaggerTool_buildUI  (bgtype)
    {
       ...
       /* TO UPDATE :::::
		...
		{
			// J Hearn
			tags.text = trim(searchField.text);
        ...
        function addItem () 
            {
				var init_text = "";
				var tag = tagsList.selection;
				if (tag != null)
				{
					if (tag.parent != tagsList)
					{
						if (!tag.parent.text.match(/other/gi))
							init_text = tag.parent.text + "_";
					}
					else if (!tag.text.match(/other/gi))
						init_text = tag.text + "_";
				}
                tagsList.selection = null;
                newTag = prompt ('Set up a new tag', init_text);
                if (newTag != null)
				{
					var parsed_tag = add_location_tag(newTag);
					var cat_node = null;
					for (var i = 0; i < tagsList.items.length; i++)
					{
						if (tagsList.items[i].text.toLowerCase() == parsed_tag.category.toLowerCase())
						{
							cat_node = tagsList.items[i];
							break;
						}
					}
					if (!cat_node)
						cat_node = tagsList.add('node', parsed_tag.category);
					var tag_item = cat_node.add('item', parsed_tag.tag);
					tag_item.expanded = true;
					tags.text = parsed_tag.full_tag.replace(/^_/, "");
				}
            }
        ...
        search_icon.addEventListener("click", addItem);
