...
function collectFiles ()
{
	...
	if (export_infos_datas['shots'].length >0)
	{
		for (obj in export_infos_datas['shots'])
		{
			...
			is_multiframe_by_shot[shot_num] = shot_datas['shot_infos'][4];
	...
	if (export_infos_datas['toExport_framing'].length > 0)
	{
		...
		for (obj in export_infos_datas['shots'])
		{
			...
			var appFolder = shot_datas['shot_infos'][2];
			var app_file_name = shot_datas['shot_infos'][3];
			var is_multi = shot_datas['shot_infos'][4];
			
			// J Hearn - make sure shot folder exists
 			var afd = new Folder(appFolder);
			if (!afd.exists)
				afd.create();
			...
			if (export_framing == true )
			{
				...
				{
					// J Hearn - decide based on is_multi how many frames to render
					for (var suffix in (is_multi ? {framingA:null, framingB:null} : {framing:null} ))
					{
						framingName = app_file_name.replace(/ae.aepx/gi, suffix);
						...
						startframe = {
							"framing": compItem.displayStartTime + compItem.duration/2,
							"framingA": compItem.displayStartTime,
							"framingB": compItem.displayStartTime + compItem.duration - 2 * durationStillFrame,
						}[suffix];
						// -- renderqueue builder
						...
						{
							alert("Cannot render framing.  Render template GUMBALL not present.  You need to load the templates.")
						...
						if (has_template  == false)
						{
							alert("Cannot render framing.  Output module GUMBALL_LO_Screenshot not present.  You need to load the output modules .")
					...
					// J Hearn - need to make sure it's a full break out
					if (!has_template) break;
					
					// J Hearn - also copy out the wide (if there's only 1 blue layer)
					if (
						bluelayers.length == 1
						&& bluelayers[0].source instanceof FootageItem
						&& bluelayers[0].source.file
						&& bluelayers[0].source.file.exists
					)
					{
						framingName = app_file_name.replace(/ae.aepx/gi, "framingWide");
						framingFile = File (decodeURI(appFolder) + "/" + framingName + '.jpg');
						if (framingFile.exists) framingFile.remove();
						
						bluelayers[0].source.file.copy(framingFile);
					}
				}
				else
				{
					app.exitCode = newErr (22, obj);
					break;
				}
